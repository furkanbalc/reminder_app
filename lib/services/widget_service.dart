...
class WidgetService {
  ...
  static const androidQualified = 'com.furkanbalci.su_hatirlatici.SuWidgetProvider';
  ...
  Future<void> init() async {
    try {
      ...
      if (Platform.isAndroid) await HomeWidget.registerInteractivityCallback(widgetBackgroundCallback);
    } catch (e) {
  ...
  Future<void> push({
    ...
    try {
      ...
      await HomeWidget.saveWidgetData<String>('next', next == null ? '' : fmtTime(next));
  ...
  Future<Uri?> initialLaunchUri() => HomeWidget.initiallyLaunchedFromHomeWidget();
}
...
Future<void> widgetBackgroundCallback(Uri? uri) async {
  ...
  try {
    ...
    final total = await repo.totalBetween(today, today.add(const Duration(days: 1)));
    final scheduler = WaterScheduler(NotificationService(), AlarmService(prefs, AlarmKitService()));
    final next = scheduler.next(settings, now, goalReachedToday: total >= settings.goalMl, lastIntakeAt: now);
    await WidgetService().push(totalMl: total, goalMl: settings.goalMl, glassMl: settings.defaultGlassMl, next: next);
  } finally {
    await db.close();
  }
}

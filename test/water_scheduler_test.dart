...
void main() {
  ...
  setUpAll(() async {
    ...
    scheduler = WaterScheduler(NotificationService(), AlarmService(prefs, AlarmKitService()));
  });
  ...
  group('WaterScheduler.slots', () {
    ...
    test('birkaç gün ileriye kurulur ama toplam sınırı aşmaz', () {
      final slots = scheduler.slots(settings, now, goalReachedToday: false);
      expect(slots.length, lessThanOrEqualTo(WaterScheduler.maxSlotsFor(AlertType.notification)));
    ...
    test('yükselen modda daha az dilim kurulur (iOS bildirim sınırı)', () {
      const esc = WaterSettings(intervalMin: 30, alertType: AlertType.escalating);
    ...
    test('az önce su içildiyse hemen sonraki dilim atlanır', () {
      ...
      final next = scheduler.next(settings, now, goalReachedToday: false, lastIntakeAt: justDrank);
    ...
    test('hafta sonu saatleri açıksa cumartesi farklı pencere kullanılır', () {
      const weekend = WaterSettings(
        activeStartMin: 8 * 60, activeEndMin: 23 * 60, intervalMin: 90,
        weekendEnabled: true, weekendStartMin: 14 * 60, weekendEndMin: 20 * 60,
      );
